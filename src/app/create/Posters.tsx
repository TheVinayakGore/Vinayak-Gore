    ...
    <>
      <main className="flex flex-col items-start w-full h-full">
        <section className="flex items-center justify-between border-b dark:border-zinc-500 shadow-lg shadow-zinc-900/[0.2] w-full">
          <h1 className="text-sky-400 text-3xl font-medium text-start relative p-5 w-1/2">
            Posters
          </h1>
        ...
        <section className="flex flex-col items-center justify-center m-auto overflow-auto w-full h-full">
          <div className="m-auto overflow-auto w-full h-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 flex-col items-center justify-center m-auto p-10 w-full">
              {allPosters.length > 0 ? (
                allPosters.map((doc) => (
                  <CardContainer
                    key={doc._id}
                    className="inter-var flex flex-col items-center justify-center m-auto w-full h-full"
                  >
                    <CardBody className="relative group/card hover:shadow-2xl hover:shadow-emerald-500/[0.1] bg-yellow-400/[0.8] rounded-xl z-40 mb-5 w-full h-full">
                      <CardItem translateZ={40} className="w-full">
                        <Image
                          ...
                          className="h-[27rem] w-full rounded-xl group-hover/card:shadow-xl"
                        />
                      </CardItem>
                      <div className="flex items-center justify-between m-auto my-5 px-5 w-full">
                        <CardItem translateZ={40} className="w-full">
                          <div className="flex items-center justify-between text-sm font-light w-full">
                            <CardItem
                              translateZ={40}
                              className="text-sm md:text-base xl:text-lg font-medium text-white"
                            >
                              {doc.title}
                            </CardItem>
                            <button
